...
public class CreatureBitmapTests {
    ...
    [Fact]
    public void NoCreatureVariantUsesAMagentaPaletteIndex() {
        // After recolor, every index any creature uses must map to a NON-magenta CreaturePalette entry ->
        // the palette covers every colour creatures use (base art + every CS recolor) and no recolor lands
        // on an unused index. Index-level (reuses no RGBA code); pixels are produced by the normal pipeline.
        string? gameDir = FindOriginalGameDir();
        if (gameDir == null) {
            return;
        }
        ...
        foreach (CreatureSprite creature in creatures.Creatures) {
            foreach (string key in creature.SpriteKeys) {
                ...
                ImageSet set = bitmapExtractor.Extract(stem + ".BMX", s);
                if (cs >= 0) {
                    CreatureColorSet.Apply(set, Lut(cs));
                }
                foreach (BmImage img in set.Images) {
                    foreach (byte index in img.BitMapData!) {
                        Assert.NotEqual(magenta, CreaturePalette.Colors[index]);
                    }
                    checkedFrames++;
                }
            }
        }
        Assert.True(checkedFrames > 200, $"expected many checked frames, got {checkedFrames}");
    }
