...
public static class DialogBranchWalker {
    ...
    /// Faithful to <c>ExecuteDialog</c> (KRONDOR.EXE 0x494bb; canassa DIALOG.C:1441-1475): an entry's
    /// <see cref="PushDialogEntryAction"/> targets go on a LIFO stack only when the entry HAS a chosen
    /// branch, the branch is followed first, and the stack is popped when a tree ends.
    /// Used for the chapter-setup dialog <c>savegame_chapter_start_dispatch → dialog_play_record(0x1e8497)</c>:
    /// entry 2000023 pushes the Var-7 (chapter) node and branches to <b>2000026</b>, so the 34 story
    /// flags 2000026 clears are applied BEFORE the popped chapter leaf's <see cref="ChangePartyAction"/>
    /// (DIAL_Z20.DDX). An id target is loaded by id in the original even inside the same file.
    ...
    public static void ExecuteActions(Dialog dialog, DialogEntry start, Func<int, int?> getGlobal,
        ...
        foreach (DialogEntry e in dialog.Entries) {
            if (e.Id is uint id) {
                // dialog_load_record_by_key takes an id key (bit 0x80000000) by id, same file or not.
                byKey[ContentKey.ForBase("dialog", (int)id)] = e;
            }
        }
        ...
        for (int hop = 0; hop < MaxHops && current != null; hop++) {
            ...
            current = ResolveContinuation(current, getGlobal, byKey, pushed);
        }
    }
